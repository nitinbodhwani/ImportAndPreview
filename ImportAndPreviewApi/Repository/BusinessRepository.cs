...
namespace ImportAndPreviewApi.Repository
{
	...
	{
		private IPersistenceRepository PersistenceRepository;
		...
		public IList<ReportData> GetReportDataByMonthAndYear(int month, int year) {
			return PersistenceRepository.GetReportDataByMonthAndYear(month, year);
		}
		...
		public IList<ReportAggregate> GetReportAggregateByDate(int day, int month, int year) {
			return PersistenceRepository.GetReportAggregateByDate(day, month, year);
		}
		...
		public IList<ReportAggregate> GetReportAggregateByCardNumberAndMonth(int cardNumber, int month, int year) {
			return PersistenceRepository.GetReportAggregateByCardNumberAndMonth(cardNumber, month, year);
		}
		...
		{
			return PersistenceRepository.SaveReportData(report);
		}
	}
}